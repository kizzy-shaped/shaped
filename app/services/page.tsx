...
function Hero({ service }: { service: IService }) {
  return (
    ...
    <div className="w-full wmin_390:h-[calc(100vh-80px)] wmax_360:!h-[65vh] wmin_360:wmax_md:!h-[70vh] bg-[#373737]  relative bg-cover bg-no-repeat wmax_lg:bg-center">
      <img
        className="w-full h-full mx-auto  z-[2] object-cover wmin_lg:hidden"
        src={service?.heroImageMobile}
        alt="" />
      <img
      ...
      {/* Overlay */}
      <div className="w-full h-full absolute top-0 left-0 z-[2] bg-black bg-opacity-50"></div>
      {/* Overlay */}
    </div>
  );
}
    ...
    <div className="w-full h-fit bg-black">
      <Container className="wmin_lg:py-[6vh] wmax_lg:py-[3vh] flex flex-col wmax_lg:items-center wmin_lg:gap-[28px] wmax_lg:gap-[14px]">
        ...
        {!service?.desc?.list ? (<div className="wmin_lg:w-[80%] wmax_lg:w-full wmin_lg:text-[18px] wmax_lg:text-[14px] font-medium text-white">
         ...
         className="wmin_lg:w-[80%] wmax_lg:w-full wmin_lg:text-[18px] wmax_lg:text-[14px] font-medium text-[#9c9c9c]"
         key={index}
         >
          {text}
        </div>))}
